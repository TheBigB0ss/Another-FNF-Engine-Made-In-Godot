...
@onready var chart_bf_anim = $character;
var curAnim = "";
var idleTimer = 0.0;
...
func _ready():
	var jsonFile = FileAccess.open("res://assets/images/editors/chart preview/bf/bf-chart.json", FileAccess.READ);
...
func _playAnim(anim=""):
	for i in animList.size():
		if animList[i] == anim:
			...
			chart_bf_anim.offset = pose_offset;
			
			chart_bf_anim.frame = 0;
			chart_bf_anim.play(posesList[i]);
			
			if animList[i].begins_with("sing"):
				idleTimer = 0;
				
			if (chart_bf_anim.frame + chart_bf_anim.frame_progress) > 2.4:
				chart_bf_anim.frame = 0;
				chart_bf_anim.frame_progress = 0;
